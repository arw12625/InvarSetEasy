...
A_W = kron([1;-1],eye(2*K));
b_W = 0.5*repmat([0;0;ones(K-4,1);0;0;0;0;ones(K-4,1);0;0],2,1);
W = Polyhedron('A', A_W,'b',b_W);
V = Polyhedron.fullSpace(0);
...
jointInputConstraintA_x = [-eye(K), zeros(K);
     zeros(K), zeros(K);
     zeros(K), -eye(K);
     zeros(K), zeros(K)];
jointInputConstraintA_u = [eye(K), zeros(K);
      -eye(K), zeros(K);
      zeros(K), eye(K);
      zeros(K), -eye(K)];
jointInputConstraintb = [zeros(K,1);
    zeros(K,1);
    zeros(K,1);
    zeros(K,1)];
...
1.5
A_ad = [[kron(eye(T+1),powerConstraintA), zeros((T+1)*size(powerConstraintA,1), 2*K*T)];
        jointInputConstraintAHor;
        %cycConstraintA
        ];
...
x0 = N/2/(K-2)*[0;0;ones(K-3,1);0;0;ones(K-3,1);0;0];
%Omega = x0 + 100*W;
%Omega = Polyhedron('V',x0');
%Omega = Polyhedron('A', powerConstraintA, 'b', powerConstraintb);

A_Omega1 = kron([1;-1],eye(2*K));
b_Omega1 = 10*repmat([0;0;ones(K-3,1);0;0;ones(K-3,1);0;0],2,1);
Omega = x0+Polyhedron('A',A_Omega1,'b',b_Omega1);
A_Omega = [Omega.A; Omega.Ae; -Omega.Ae];
b_Omega = [Omega.b; Omega.be; -Omega.be];

A_ad = [A_ad; 
        zeros(size(A_Omega,1),2*K*T),A_Omega,zeros(size(A_Omega,1),2*K*T)];
b_ad = [b_ad; b_Omega];
