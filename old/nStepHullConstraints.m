function [Constraints] = nStepHullConstraints(Omega, X, U, N, A, B, alpha, x)
%nStepHullConstraints Generates the linear constraints specifying the
%convex hull of the union of the first N backwards step sets of Omega
...
for k = 1:N
    Constraints = [Constraints;
        H * stepMap(k, A, B, z(:,k), v(:, :, k)) <= alpha * lambda(k) * h;
        F * z(:,k) <= lambda(k) * f;
    ];
    for j = 1:k
        Constraints = [Constraints;
            F * stepMap(j, A, B, z(:,k), v(:,:,j)) <= lambda(k) * f;
            G * v(:, j, k) <= lambda(k) * g;
        ];
    end
end
